...
export class TagService {
  ...
  async findAll(queryParams): Promise<Tag[]> {
    ...
    if (postContentStatus) {
      ...
      tags.leftJoinAndSelect('tag.postContents', 'postContents');
    }

    const data = await tags.getMany();

    data.forEach((ele: any) => {
      Object.assign(ele, { postContentCount: ele.postContents.length });
      delete ele.postContents;
    });
  ...
  async getPostContentById(id, status = null): Promise<Tag> {
    const data = (await this.tagRepository
      .createQueryBuilder('tag')
      .leftJoinAndSelect('tag.postContents', 'postContents')
      .orderBy('postContents.updateAt', 'DESC')
      .where('tag.id=:id')
      .orWhere('tag.label=:id')
      .orWhere('tag.value=:id')
      .setParameter('id', id)
    ...
    if (status) {
      data.postContents = data.postContents.filter(a => a.status === status);
      return data;
    }
    return data;
  }
